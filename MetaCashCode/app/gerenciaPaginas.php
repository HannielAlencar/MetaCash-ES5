...
try {
    // 1. Precisamos do CNPJ da empresa para localizar a linha na tabela 'empresa_config'
    $stmt_cnpj = $pdo->prepare("SELECT cnpj FROM empresas WHERE id_empresa = :id_empresa");
    $stmt_cnpj->execute([':id_empresa' => $id_empresa]);
    $empresa = $stmt_cnpj->fetch(PDO::FETCH_ASSOC);
    ...
    // 2. Tenta atualizar a linha existente pelo CNPJ
    $stmt = $pdo->prepare("UPDATE empresa_config SET config_dashboard = :config, updated_at = CURRENT_TIMESTAMP WHERE cnpj = :cnpj");
    $stmt->execute([':config' => $json_recebido, ':cnpj' => $cnpj]);

    // 3. Se nenhuma linha foi atualizada (rowCount 0), inserimos uma nova
    if ($stmt->rowCount() === 0) {
        $stmt_ins = $pdo->prepare("INSERT INTO empresa_config (usuario_id, nome_empresa, cnpj, config_dashboard) VALUES (:uid, :nome, :cnpj, :config)");
        $stmt_ins->execute([
            ':uid' => $_SESSION['usuario_id'],
            ':nome' => 'Empresa ID: ' . $id_empresa, // Ou busque o nome real
            ':cnpj' => $cnpj,
            ':config' => $json_recebido
        ]);
    }
