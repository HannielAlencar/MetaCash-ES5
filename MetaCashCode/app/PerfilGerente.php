<?php
// 1. O PHP DEVE COMEÇAR NO TOPO ABSOLUTO, SEM ESPAÇOS ANTES DA TAG <?php
...
$email_usuario = $_SESSION['email_usuario'] ?? 'gerente@empresa.com';
$matricula_usuario = $_SESSION['matricula'] ?? '2024001';
$cpf_usuario = $_SESSION['cpf_usuario'] ?? '123.456.789-00';
$papel_usuario = $_SESSION['nivel_permissao'] ?? 'Gerente';

// Processamento dos Formulários (POST) antes de qualquer HTML
...
$iniciais = (function_exists('mb_substr')) ? mb_strtoupper(mb_substr($nome_usuario, 0, 1)) : strtoupper(substr($nome_usuario, 0, 1));
?>
<!DOCTYPE html>
<html lang="pt-br">
<head>
...
<body class="bg-meta-fundo text-slate-800">
    ...
    <div class="flex min-h-screen">
        ...
        <main class="flex-1 p-10 ml-64 w-full">
            <header class="mb-8">
                <a href="javascript:history.back()" class="text-sm text-slate-500 hover:text-meta-clara flex items-center gap-2 mb-4 transition">
            ...
            <div class="max-w-5xl space-y-8">
                ...
                <section class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                    <div class="p-6 border-b border-slate-100 flex items-center gap-3">
                        <div class="w-8 h-8 bg-blue-50 text-meta-clara rounded-lg flex items-center justify-center">
                    ...
                    <div class="p-8">
                        <div class="flex items-center gap-4 mb-8">
                            <div class="w-20 h-20 bg-gradient-to-br from-teal-500 to-emerald-600 rounded-full flex items-center justify-center text-[#0f172a] text-3xl font-bold shadow-md">
                            ...
                            <div>
                                ...
                                <span class="inline-block bg-blue-100 text-meta-clara text-[10px] font-bold px-2 py-0.5 rounded-full uppercase mb-1">
                        ...
                        <form class="space-y-6" method="POST" action="PerfilGerente.php">
                            ...
                            <div class="grid grid-cols-1 gap-6">
                                ...
                                <div>
                                    ...
                                    <div class="w-full px-4 py-3 rounded-xl bg-sky-100 border border-sky-200 text-meta-clara flex items-center gap-3">
                                        <i class="fas fa-user-tag text-meta-clara"></i>
                                ...
                                <div>
                                    ...
                                    <div class="relative">
                                        ...
                                        <input type="email" name="email" value="<?= htmlspecialchars($email_usuario, ENT_QUOTES, 'UTF-8') ?>" class="w-full pl-11 pr-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-meta-destaque outline-none transition">
                                    </div>
                                    <p class="text-[10px] text-meta-clara mt-1 italic"><i class="fas fa-check"></i> Este campo pode ser alterado</p>
                                </div>
                            </div>

                            <div class="flex justify-end pt-4">
                                <button type="submit" class="bg-gradient-to-r from-slate-800 to-meta-btn1 text-white px-8 py-3 rounded-xl font-bold shadow-lg hover:opacity-90 transition flex items-center gap-2">
                            ...
                            <div class="grid grid-cols-1 gap-6">
                                <div>
                                    ...
                                    <div class="relative">
                                        ...
                                        <input id="current_password" type="password" name="current_password" placeholder="Digite sua senha atual" class="w-full pl-11 pr-12 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-meta-destaque outline-none transition">
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <div class="relative">
                                        <i class="fas fa-unlock-alt absolute left-4 top-1/2 -translate-y-1/2 text-slate-300"></i>
                                        <input id="new_password" type="password" name="new_password" placeholder="Digite sua nova senha" class="w-full pl-11 pr-12 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-meta-destaque outline-none transition">
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <div class="relative">
                                        <i class="fas fa-unlock-alt absolute left-4 top-1/2 -translate-y-1/2 text-slate-300"></i>
                                        <input id="confirm_password" type="password" name="confirm_password" placeholder="Confirme sua nova senha" class="w-full pl-11 pr-12 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-meta-destaque outline-none transition">
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="flex flex-col md:flex-row justify-between items-end gap-6 pt-4">
                                <div class="bg-slate-50 p-4 rounded-xl border border-slate-200 w-full md:w-fit">
                                    ...
                                    <div class="grid grid-cols-2 gap-x-8 gap-y-2">
                                        <div class="text-[10px] text-slate-500 flex items-center gap-2"><i class="fas fa-times text-red-500"></i> Mínimo de 8 caracteres</div>
                                        <div class="text-[10px] text-slate-500 flex items-center gap-2"><i class="fas fa-times text-red-500"></i> Letra maiúscula (A-Z)</div>
                                        <div class="text-[10px] text-slate-500 flex items-center gap-2"><i class="fas fa-times text-red-500"></i> Letra minúscula (a-z)</div>
                                        <div class="text-[10px] text-slate-500 flex items-center gap-2"><i class="fas fa-times text-red-500"></i> Número (0-9)</div>
                                        <div class="text-[10px] text-slate-500 flex items-center gap-2"><i class="fas fa-times text-red-500"></i> Caractere especial (!@#$...)</div>
                                    </div>
                                </div>
                                <button type="submit" class="bg-slate-100 text-slate-600 px-6 py-2.5 rounded-lg text-sm hover:bg-slate-200 transition duration-300 flex items-center gap-2 border border-slate-200 shadow-sm">
                                    <i class="fas fa-key text-xs text-meta-clara"></i> Alterar Senha
                                </button>
                            </div>
                        </form>
                    </div>
    ...
    <script>
    // Validação básica do lado do cliente para as senhas
    (function(){
        const form = document.querySelector('input[name="form_type"][value="password"]')?.closest('form');
        if (!form) return;

        const errDiv = document.createElement('div');
        errDiv.id = 'password-client-error';
        errDiv.style.display = 'none';
        errDiv.className = 'mb-4 px-4 py-3 rounded-lg text-sm text-red-700 bg-red-50 border border-red-200';
